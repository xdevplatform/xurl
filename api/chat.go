...
// GetChatConversation fetches one conversation's metadata and returns every
// participant/member user id found on it, plus the raw response.
func GetChatConversation(client Client, conversationID string, opts RequestOptions) ([]string, json.RawMessage, error) {
	opts.Method = "GET"
	...
	var ids []string
	for _, group := range [][]string{out.Data.ParticipantIDs, out.Data.MemberIDs, out.Data.AdminIDs} {
